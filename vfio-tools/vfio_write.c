...
int main() {
	...
	if(ret < 0){
		...
		printf("Will try to read Cache Line Size in config space\n.");
		void * buf = malloc(1);
		unsigned char * val = (unsigned char *) buf;
		ret = pread(device, buf, sizeof(buf), reg.offset+12);
		if(ret < 0){
			printf("Read unsuccessful %d\n", ret);
		}
		else{
			printf("Value read: %x\n", *val);
		}

		printf("\n\nWill try to write to CLS\n;");
		*val = 11;
		ret = pwrite(device, buf, sizeof(buf), reg.offset+12);
		if(ret < 0){
			printf("Write unsuccessful %d\n", ret);
		}
		else{
			printf("Wrote value %x\n", *val);
		}

		printf("\n\nWill try to read again to see if changes took hold.\n");
		buf = malloc(1);
		...
		if(ret < 0){
			...
			printf("New Value read: %x\n", *val);
		}

		buf = malloc(1);
		val = (unsigned char *) buf;
		ret = pread(device, buf, sizeof(buf), reg.offset);
		if(ret < 0){
			printf("Read unsuccessful %d\n", ret);
		}
		else{
